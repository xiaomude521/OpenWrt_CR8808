...
# 5. 修复 luci-app-store 的依赖问题
echo "Fixing luci-app-store dependencies..."
if [ -f "package/lean/luci-app-store/Makefile" ]; then
    # 检查 Makefile 中是否有对 luci-lib-taskd 的依赖
    if grep -q "luci-lib-taskd" "package/lean/luci-app-store/Makefile"; then
        echo "Found luci-lib-taskd dependency in luci-app-store/Makefile"
        
        # 如果 luci-lib-taskd 不存在，则移除这个依赖
        if [ ! -d "package/lean/luci-lib-taskd" ]; then
            echo "Removing luci-lib-taskd dependency as it's not available"
            sed -i '/luci-lib-taskd/d' "package/lean/luci-app-store/Makefile"
            
            # 还需要移除对应的 PKG_BUILD_DEPENDS
            sed -i '/PKG_BUILD_DEPENDS.*luci-lib-taskd/d' "package/lean/luci-app-store/Makefile"
        fi
    fi
fi
...
# 安装常见工具
./scripts/feeds install lm-sensors 2>/dev/null || echo "lm-sensors not available in feeds"
./scripts/feeds install pciutils 2>/dev/null || echo "pciutils not available in feeds"
./scripts/feeds install bash 2>/dev/null || echo "bash not available in feeds"
./scripts/feeds install jq 2>/dev/null || echo "jq not available in feeds"
./scripts/feeds install curl 2>/dev/null || echo "curl not available in feeds"
./scripts/feeds install wget-ssl 2>/dev/null || echo "wget-ssl not available in feeds"

# 安装网络服务相关
./scripts/feeds install wsdd2 2>/dev/null || echo "wsdd2 not available in feeds"
./scripts/feeds install luci-app-ksmbd 2>/dev/null || echo "luci-app-ksmbd not available in feeds"
./scripts/feeds install luci-app-samba 2>/dev/null || echo "luci-app-samba not available in feeds"
./scripts/feeds install luci-app-samba4 2>/dev/null || echo "luci-app-samba4 not available in feeds"

# 安装系统库
./scripts/feeds install libpam 2>/dev/null || echo "libpam not available in feeds"
./scripts/feeds install libkmod 2>/dev/null || echo "libkmod not available in feeds"
./scripts/feeds install liblzma 2>/dev/null || echo "liblzma not available in feeds"
./scripts/feeds install libpcre2 2>/dev/null || echo "libpcre2 not available in feeds"

# 安装 OpenWrt/Luci 相关
./scripts/feeds install luci-compat 2>/dev/null || echo "luci-compat not available in feeds"
./scripts/feeds install luci-proto-ipv6 2>/dev/null || echo "luci-proto-ipv6 not available in feeds"
./scripts/feeds install ddns-scripts 2>/dev/null || echo "ddns-scripts not available in feeds"

# 7. 移除有问题的包（如果不需要）
echo "Removing problematic packages if not needed..."
...
# 如果您不需要 fogvdn 相关功能：
# rm -rf package/lean/fogvdn 2>/dev/null || true
# rm -rf package/lean/luci-app-fogvdn 2>/dev/null || true

# 如果您不需要 leigod 加速器：
# rm -rf package/lean/luci-app-leigod-acc 2>/dev/null || true
